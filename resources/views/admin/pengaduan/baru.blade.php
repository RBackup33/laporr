<x-layout>
    <div class="p-6 sm:p-8">
        <h1 class="text-2xl font-semibold text-gray-800 mb-6">Daftar Laporan</h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @if ($hilmi_pengaduan->count() > 0)
                @foreach ($hilmi_pengaduan as $hilmi_item)
                    <div
                        ...
                        <div class="relative h-56 overflow-hidden">
                            @if ($hilmi_item->foto)
                                <img src="{{ asset('storage/' . $hilmi_item->foto) }}" alt="Foto Laporan"
                            ...
                            <div
                                ...
                                @if ($hilmi_item->status == '0') bg-yellow-500 text-white
                                @elseif($hilmi_item->status == 'proses')
                                    bg-blue-500 text-white
                                @else
                                    bg-green-500 text-white @endif">
                                        @if ($hilmi_item->status == '0')
                                            Baru
                                        @elseif($hilmi_item->status == 'proses')
                                            Proses
                                        @else
                            ...
                            <div class="flex items-center justify-between mb-4">
                                <div class="flex items-center gap-4">
                                    <div
                                        ...
                                        {{ strtoupper(substr($hilmi_item->masyarakat->nama, 0, 1)) }}
                                    </div>
                                    <div>
                                        <h3 class="font-semibold text-gray-800 text-lg">
                                            {{ $hilmi_item->masyarakat->nama }}
                                        </h3>
                                        <p class="text-sm text-gray-500">
                                            {{ \Carbon\Carbon::parse($hilmi_item->tgl_pengaduan)->format('d M Y') }}</p>
                                    </div>
                                </div>
                            </div>
                            <p class="text-gray-700 mb-4 line-clamp-3">{{ $hilmi_item->isi_laporan }}</p>

                            <div class="mt-6 space-y-4">
                                <div class="flex items-center justify-between">
                                    <button onclick="openDetailModal('{{ $hilmi_item->id_pengaduan }}')"
                                ...
                                @if ($hilmi_item->status == '0')
                                    <div class="flex justify-between items-center space-x-2">
                                        <form action="/admin/{{ $hilmi_item->id_pengaduan }}/terima" method="POST"
                                            class="w-1/2">
                                            @csrf
                                            <button type="submit"
                                        ...
                                        <button onclick="showAlert('tolak', {{ $hilmi_item->id_pengaduan }})"
                                ...
                                @if ($hilmi_item->status != 'selesai')
                                    <button onclick="showResponseForm({{ $hilmi_item->id_pengaduan }})"
            ...
            fetch(`/api/pengaduan/${id_pengaduan}`)
                ...
                .then(data => {
                    if (data.message) {
                        ...
                        ${data.foto ? `
                                    <div class="mt-6">
                                        <p class="text-sm text-gray-600 mb-2">Foto laporan</p>
                                        <img src="/storage/${data.foto}" alt="Foto laporan" class="w-full h-auto rounded-lg shadow-md">
                                    </div>
                                ` : ''}
                    `;
                    }
                ...
                .catch(error => {
                    detailContent.innerHTML = '<p class="text-red-500 text-center">Gagal memuat detail laporan.</p>';
