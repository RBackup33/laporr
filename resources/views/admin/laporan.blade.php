<x-layout>
    <div class="container mx-auto px-4 py-8">
        <h1 class="text-4xl font-bold mb-8 text-gray-800">Celapora</h1>
                        ...
                        @foreach ($hilmi_laporan as $hilmi)
                            <tr class="hover:bg-gray-50 transition duration-150 ease-in-out">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                    {{ $hilmi->id_pengaduan }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{  \Carbon\Carbon::parse($hilmi->tgl_pengaduan)->format('d M Y') }}</td>
                                    ...
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-10 w-10">
                                            <span
                                                ...
                                                {{ strtoupper(substr($hilmi->masyarakat->nama, 0, 1)) }}
                                            </span>
                                        </div>
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-900">
                                                {{ $hilmi->masyarakat->nama }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900 max-w-xs overflow-hidden overflow-ellipsis">
                                        {{ Str::limit($hilmi->isi_laporan, 50) }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                    @if ($hilmi->status == '0') bg-yellow-100 text-yellow-800
                                    @elseif($hilmi->status == 'proses') bg-blue-100 text-blue-800
                                    @else bg-green-100 text-green-800 @endif">
                                        {{ $hilmi->status == '0' ? 'Baru' : ucfirst($hilmi->status) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <button onclick="printReport({{ $hilmi->id_pengaduan }})"
    ...
    <script>
        ...
        function updateTable(hilmi_laporan) {
            const tableBody = document.getElementById('reportTableBody');
            tableBody.innerHTML = ''; // Kosongkan isi tabel

            if (hilmi_laporan.length === 0) {
            ...
            hilmi_laporan.forEach(complaint => {
